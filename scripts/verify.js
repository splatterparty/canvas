...
(async () => {
  ...
  if (target === 'lamina1_mainnet') {
    console.log('Skipping verification on lamina1_mainnet') // TODO: currently not available
    return
  }
  ...
  for (let { name, onChain } of contracts) {
    ...
    //await $`forge verify-contract ${onChain.address} ${name} --constructor-args ${args} --chain-id ${deploymentInfo[target].chainId} --verifier blockscout --verifier-url https://explorer.lamina1.com/api? --watch`
