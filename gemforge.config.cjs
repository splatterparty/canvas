...
module.exports = {
  ...
  commands: {
    // the build command
    build: "forge build --names --sizes --optimize --via-ir",
  },
  paths: {
  ...
  wallets: {
    ...
    wallet_lamina1: { 
      type: "private-key",
      config: {
        key: () => process.env.LAMINA1_PRIVATE_KEY
      }
    }
  },
  ...
  networks: {
    ...
    lamina1_mainnet: {
      // RPC endpoint URL
      rpcUrl: "https://subnets.avax.network/lamina1/mainnet/rpc",
    },

    lamina1_fuji_testnet: {
      // RPC endpoint URL
    ...
    lamina1_betanet: {
      // RPC endpoint URL
      rpcUrl: "https://rpc-betanet.lamina1.com/ext/bc/C/rpc",
    },
  },
  // Targets to deploy
  targets: {
    ...
    lamina1_mainnet: {
      // Network to deploy to
      network: "lamina1_mainnet",
      // Wallet to use for deployment
      wallet: "wallet_lamina1",
      // Initialization function arguments
      initArgs: [],
      // CREATE3 salt
      create3Salt: SALT,
